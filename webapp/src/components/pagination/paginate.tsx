...
import { PaginationChangeEventData } from '@/types/common';
    ...
    <Fragment>
      <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
        {pages.map((page, index) => {
          if (page === LEFT_PAGE) {
            return (
              <button key={index} onClick={handleMoveLeft} className={arrowClasses}>
          ...
          if (page === RIGHT_PAGE) {
            return (
              <button key={index} onClick={handleMoveRight} className={arrowClasses}>
          ...
          return (
            <button key={index} onClick={handleClick(page)} className={pageClasses(index, +page, pages.length)}>
              {page}
            </button>
          );
        })}
